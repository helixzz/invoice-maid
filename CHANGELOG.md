...
**Invoice taxonomy + SaaS portal scraping.** Two independent tracks shipped together:

- **Track A** — widens invoice acceptance from "Chinese VAT only" to a 5-category taxonomy (`vat_invoice`, `saas_invoice`, `receipt`, `proforma`, `other`). Non-VAT invoices (Cursor / OpenAI / Stripe subscriptions, receipts, proforma quotes, etc.) now save with a correct category tag instead of being rejected as `not_vat_invoice`. Complete backend plumbing (migration, ORM, FTS5, API filters, stats breakdown, CSV column) plus frontend filter chips, category badges, and a by-category stats panel.
- **Track B** — Playwright-based SaaS portal scraper, Cursor-first. Feeds the same parse → LLM enrich → dedupe → save pipeline used by email and manual uploads, with Fernet-encrypted credentials and automatic Playwright `storage_state` round-tripping so operators only log in once. Designed open-closed (`BaseScraper` + `ScraperFactory`) so Stripe / OpenAI / any other vendor adds 1 file + 1 registry line.

Zero breaking changes. Existing Chinese VAT invoices, Outlook delta scanning, webhooks, admin panel, and everything else from v1.1.0 behave identically. Operators who want v1.1.x-style strict VAT-only acceptance set `STRICT_VAT_ONLY=true`.

### Added

#### Track A — Invoice category taxonomy

- **`InvoiceCategory` enum** with five values: `vat_invoice`, `saas_invoice`, `receipt`, `proforma`, `other`. Defined as a Python `str, Enum` in `app/schemas/invoice.py` and surfaced as a `Literal` union in `frontend/src/types/index.ts` so backend and frontend cannot drift.
- **Migration `0015_add_invoice_category`** adds a `NOT NULL` `invoice_category` column with `server_default='vat_invoice'` (backfill-safe — every pre-v1.2.0 invoice becomes `vat_invoice`), plus an index on `(user_id, invoice_category)` for filter performance. Rebuilds the FTS5 `invoices_fts` virtual table + triggers to include the category as a searchable token.
- **LLM prompt STEP 0.5** (`app/prompts/extract_invoice.txt`) — new "CATEGORY CLASSIFICATION" step decides the category from document signals (Chinese VAT markers → `vat_invoice`; `stripe.com` / `cursor.com` / `openai.com` / non-CNY currency → `saas_invoice`; `收据` / `Receipt` → `receipt`; `Proforma Invoice` / `NOT A TAX INVOICE` → `proforma`; anything else legitimate → `other`). STEP 1 VAT-validity check is now scoped to `invoice_category == vat_invoice`; non-VAT categories correctly save with `is_valid_tax_invoice=false` and the category tag rather than getting rejected.
- **5 synthetic PDF fixtures** (`backend/tests/fixtures/invoice_samples/`) — one per category, generated by `tests/fixtures/generate_category_samples.py` (reportlab + Noto Sans CJK).
- **Live-LLM prompt validation harness** (`backend/tests/test_prompts/test_extract_categories.py`) — gated on `INVOICE_MAID_LIVE_LLM_TESTS=1`; runs each fixture through the real LLM to assert classification correctness. Structural guard runs unconditionally in CI so renaming reportlab APIs can't silently break the generator.
- **List filter** — `GET /api/v1/invoices?category=vat_invoice&category=saas_invoice` (repeatable array param). Passes through to `app/services/search_service.py` as a `categories: list[str] = None` parameter.
- **Stats breakdown** — `GET /api/v1/stats` now returns `by_category: list[CategoryCountPoint]` so the dashboard can render a per-category count tile. Zero-count categories are included so the UI can render "— (0)" placeholders rather than disappearing.
- **CSV export** — the `invoice_category` column is included in the exported CSV, ordered immediately before `invoice_type`. CSV export also accepts the `?category=...` filter.
- **Test-helpers endpoint** — `POST /api/v1/test-helpers/seed-invoice-category-mix` seeds one invoice per category (5 total) for Playwright E2E runs. Gated behind `ENABLE_TEST_HELPERS=true` and never mounted in production.
- **Frontend — filter chips + badges + stats**:
  - `frontend/src/components/MultiSelectChips.vue` — reusable `v-model` chip-style multi-select with `aria-pressed` and Enter/Space keyboard toggle.
  - `frontend/src/components/CategoryBadge.vue` — colored pill (blue / violet / amber / yellow / slate, kept disjoint from the existing confidence-badge palette).
  - `InvoiceListView.vue` — category chip bar in the filter panel, new Category column in the table, by-category stats panel alongside the existing dashboard tiles, `?category=` URL query sync so selections survive reload and browser back/forward.
  - New Playwright spec `frontend/e2e/invoice-category-filter.spec.ts` — asserts chip filtering, badge rendering, stats panel, and URL persistence. All 19 frontend E2E specs pass.

#### Track B — SaaS portal scraping (Cursor-first)

- **`BaseScraper` ABC** (`app/services/scrapers/base.py`) — defines the async `scan(db, account, *, options) → list[RawEmail]` contract. Concrete scrapers expose the same side-effect attributes (`_last_scan_state`, `_scan_events`, `_updated_storage_state`) as the existing email scanners, so the scheduler drain pass and `ExtractionLog` wiring work unchanged.
- **`PlaywrightSession` async context manager** (`app/services/scrapers/playwright_session.py`) — launches headless Chromium, loads a JSON `storage_state` if present, exposes the authenticated browser context, and round-trips the updated `storage_state` on exit so session renewals persist across scans. Raises `PlaywrightUnavailableError` cleanly if the `playwright` package is not installed; importers of `app.services.scrapers` do **not** crash when playwright is absent (`PLAYWRIGHT_AVAILABLE` flag + lazy import).
- **`CursorScraper`** (`app/services/scrapers/cursor.py`) — navigates `cursor.com/dashboard` → billing → invoices list, iterates rows, downloads each PDF via the authenticated session, yields one `RawEmail`-shaped payload per invoice with the PDF bytes as a synthetic attachment. Selectors centralized as module-level constants for easy re-targeting. Optional TOTP 2FA via stored Fernet-encrypted `totp_secret_encrypted`.
- **`ScraperFactory`** (`app/services/scrapers/factory.py`) — open-closed registry keyed on `account.type`. `register("cursor", CursorScraper)` at package import time; adding Stripe / OpenAI later is one line.
- **Scheduler dispatch** — `scan_all_accounts` now routes `EmailAccount.type == "cursor"` (and any future `ScraperFactory.is_scraper_type(...)` hit) to the scraper path. Returned `RawEmail` payloads flow into the existing parse → LLM enrich → dedupe → save pipeline with zero branching downstream. After a successful scrape, `scanner._updated_storage_state` is Fernet-encrypted and persisted back to `email_accounts.playwright_storage_state`.
- **Migration `0016_add_scraper_account_fields`** adds 4 nullable TEXT columns to `email_accounts`: `playwright_storage_state` (plaintext per design §4.2 — cookies are bearer tokens already; rotation > encryption value), `secondary_credential_encrypted` (Fernet, JWT_SECRET-derived key), `secondary_password_encrypted` (Fernet), `totp_secret_encrypted` (Fernet). Round-trip upgrade / downgrade clean.
- **`email_accounts` API** accepts and updates the 4 new fields on POST / PUT. Responses expose `has_secondary_credential` / `has_secondary_password` / `has_totp_secret` / `has_playwright_storage_state` boolean flags only — plaintext is never returned from reads.
- **`scripts/cursor_login_local.py`** — operator-facing CLI helper. Opens a headful Chromium against `cursor.com/login`, waits for the operator to complete login + 2FA interactively, captures the `storage_state` JSON, prints it to stdout for paste into the Settings → Cursor account form.
- **31 new backend tests** (701 → 732, still 100% line + branch coverage):
  - 3 migration tests for 0015 (up + down + FTS5 rebuild) were already in v1.2.0-alpha; 3 more for 0016 (up + down + nullable-columns invariant).
  - 13 Cursor scraper tests per the §8.2 matrix: storage-state round-trip, selector mismatch, empty invoices page, PDF download, TOTP 2FA success, TOTP wrong code, missing TOTP secret, expired storage_state login retry, persistent login failure `auth_required` emission, network timeout, partial invoice list on error, scheduler drain `auth_required` → `ExtractionLog`, storage-state persistence across a scheduler run.
  - 3 `PlaywrightSession` tests (fresh session / storage_state load / playwright absent).
  - 2 `ScraperFactory` tests (register + lookup, unknown type raises).
  - 3 degradation tests (scraper returns `[]` when playwright absent, scheduler persists updated storage_state, base `test_connection` default).
  - 2 `BaseScraper` + 2 extra factory edge tests, plus 3 `email_accounts` API tests covering POST / PUT shape for the 4 new fields.
- **5 new frontend E2E cases** across 2 specs (`invoice-category-filter.spec.ts`): chip filter round-trip, badge rendering, stats panel, CSV export includes category, `?category=` URL persistence.

### Changed

- **Rejection rule** (`app/services/manual_upload.py`, `app/tasks/scheduler.py`) — the `low_confidence` / `not_vat_invoice` reject branch now fires **only when `invoice_category == 'vat_invoice'` AND validity failed**. Non-VAT categories save with `is_valid_tax_invoice=false` + category tag. Operators who want v1.1.x strictness set `STRICT_VAT_ONLY=true` (new env var, default `false`) to revert: every non-`vat_invoice` classification then rejects with `not_vat_invoice`, byte-identical to the v1.1.x behavior.
- **LLM merge extension** — the deterministic-parser vs. LLM-extract merge block now fires when `is_valid_tax_invoice=True` **OR** `invoice_category != vat_invoice`. Without this extension, SaaS invoices were extracting with `is_valid_tax_invoice=False`, skipping merge, leaving `parsed.invoice_no` unset, and being rejected as `low_confidence`. Documented inline in both `manual_upload.py:~322-356` and `scheduler.py:~513-556`.
- **Frontend `exportInvoicesCSV` path** — repaired pre-existing 404 (`/invoices/export/csv` → `/invoices/export`). Backend has always exposed `/export`; the CSV button has been 404-ing silently since the endpoint was added. Fixed as a one-line repair inside the A8 commit, unrelated to the category work but found while touching the call site.

### Not Changed

- **Outlook delta scanning** (v1.1.0) — untouched. Delta query path + kill-switch preserved.
- **Email scanner interface** — scrapers bolt on alongside via the `ScraperFactory.is_scraper_type()` branch in `scan_all_accounts`; existing IMAP / POP3 / QQ / Outlook paths are byte-identical.
- **Multi-user tenant isolation** (v0.9.0) — preserved. Scrapers honour `user_id` exactly like email scanners.
- **Webhooks, admin panel, rate limiting, health endpoint** — unchanged.
- **No backend dependency version bumps** beyond adding `playwright>=1.45,<2.0` to `[project] dependencies` and `reportlab>=4.0,<5.0` to `[project.optional-dependencies] dev`.

### Upgrade path

Zero-touch from v1.1.0:

1. `sudo /usr/local/sbin/invoice-maid-upgrade --tag v1.2.0` runs `alembic upgrade head` (applies 0015 + 0016; existing invoices backfill to `invoice_category='vat_invoice'`), restarts the service, and waits for `/api/v1/health` to report `"version": "1.2.0"`.
2. **For Cursor scraping (optional):** install the Playwright Chromium browser binary on the host (`playwright install chromium` as the invoice-maid user), run `scripts/cursor_login_local.py` on any workstation, paste the captured `storage_state` JSON into Settings → add Cursor account.
3. **Rollback:** `sudo /usr/local/sbin/invoice-maid-upgrade --tag v1.1.0` runs `alembic downgrade -2` and restores v1.1.0. Cursor accounts, their stored credentials, and any `saas_invoice` / `receipt` / `proforma` / `other` invoices are dropped on downgrade (migration 0015's down side uses the FTS5 rebuild pattern); back up before rolling back if you have invoices in those categories.

### Deferred to v1.2.1+

- **Real-Cursor manual QA (§8.3 B13)** — the Cursor scraper is shipped with a fully mocked 18-test matrix; end-to-end manual QA against the real Cursor dashboard runs post-deploy and depends on operator credentials.
- **Stripe / OpenAI scrapers** — `BaseScraper` + `ScraperFactory` are ready; adding each vendor is one scraper file + one `register()` call.
- **Settings UI for Cursor account type** — the backend accepts the Cursor account shape today; the Settings form currently exposes email-only types. Cursor accounts are created via the API directly in v1.2.0. UI form follows in v1.2.1.
- **Separate `TOTP_ENCRYPTION_KEY`** — Track B design §4.3 argued for rotating the TOTP key independently of JWT. For v1.2.0 all three `*_encrypted` fields share the JWT_SECRET-derived Fernet key to match the existing `password_encrypted` pattern. A key-rotation migration can land in v1.2.1 without schema change.
