...
const SEED_INVOICE_IDS = ['CATMIX-VAT-001', 'CATMIX-SAAS-001', 'CATMIX-RCPT-001', 'CATMIX-PROF-001', 'CATMIX-OTHR-001'];
...
test.describe('Invoice category filter (v1.2.0 A11)', () => {
  test.beforeEach(async ({ request }) => {
    ...
    expect(body.categories).toEqual(['vat_invoice', 'saas_invoice', 'receipt', 'proforma', 'other']);
  });
  ...
  test('renders badges, narrows rows via chips, and shows by_category stats', async ({ page }) => {
    ...
    for (const category of ['vat_invoice', 'saas_invoice', 'receipt', 'proforma', 'other']) {
    ...
    const saasChip = filter.locator('button[data-chip-value="saas_invoice"]');
    const vatChip = filter.locator('button[data-chip-value="vat_invoice"]');

    await saasChip.click();
    await expect(saasChip).toHaveAttribute('aria-pressed', 'true');
    await expect(page).toHaveURL(/category=saas_invoice/);
    await expect(dataRows).toHaveCount(1);
    await expect(page.getByRole('row').filter({ hasText: 'CATMIX-SAAS-001' })).toBeVisible();
    ...
    await saasChip.click();
    await vatChip.click();
    await expect(saasChip).toHaveAttribute('aria-pressed', 'false');
