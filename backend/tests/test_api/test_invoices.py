...
async def test_list_invoices_with_single_category_narrows_results(
    ...
    """v1.2.0 Track A: ?category=overseas_invoice returns only that category."""
    await create_invoice(invoice_no="INV-VAT-1", invoice_category="vat_invoice", email_uid="u1")
    await create_invoice(invoice_no="INV-OVERSEAS-1", invoice_category="overseas_invoice", email_uid="u2")
    await create_invoice(invoice_no="INV-RCPT-1", invoice_category="receipt", email_uid="u3")

    response = await client.get(
        "/api/v1/invoices?category=overseas_invoice", headers=auth_headers
    )
    assert response.status_code == 200
    body = response.json()
    assert body["total"] == 1
    assert body["items"][0]["invoice_no"] == "INV-OVERSEAS-1"
    assert body["items"][0]["invoice_category"] == "overseas_invoice"
...
async def test_list_invoices_with_multi_category_returns_union(
    ...
    await create_invoice(invoice_no="INV-OVERSEAS-2", invoice_category="overseas_invoice", email_uid="u2")
    await create_invoice(invoice_no="INV-RCPT-2", invoice_category="receipt", email_uid="u3")

    response = await client.get(
        "/api/v1/invoices?category=overseas_invoice&category=receipt", headers=auth_headers
    )
    assert response.status_code == 200
    body = response.json()
    assert body["total"] == 2
    returned_nos = {item["invoice_no"] for item in body["items"]}
    assert returned_nos == {"INV-OVERSEAS-2", "INV-RCPT-2"}
...
async def test_list_invoices_without_category_returns_all(
    ...
    await create_invoice(invoice_no="INV-OVERSEAS-3", invoice_category="overseas_invoice", email_uid="u2")
...
async def test_list_invoices_with_category_and_search_query_both_apply(
    ...
    """FTS branch: ?q=Alpha + ?category=overseas_invoice both filter."""
    await create_invoice(
        invoice_no="INV-ALPHA-OVERSEAS",
        invoice_category="overseas_invoice",
        buyer="Alpha",
        raw_text="Alpha",
        email_uid="u1",
    )
    ...
    response = await client.get(
        "/api/v1/invoices?q=Alpha&category=overseas_invoice", headers=auth_headers
    )
    assert response.status_code == 200
    body = response.json()
    assert body["total"] == 1
    assert body["items"][0]["invoice_no"] == "INV-ALPHA-OVERSEAS"


async def test_export_invoices_respects_category_filter(
    client, auth_headers, create_invoice
) -> None:
    """CSV export ?category=overseas_invoice returns only those rows."""
    await create_invoice(invoice_no="INV-VAT-E", invoice_category="vat_invoice", email_uid="u1")
    await create_invoice(invoice_no="INV-OVERSEAS-E", invoice_category="overseas_invoice", email_uid="u2")

    response = await client.get(
        "/api/v1/invoices/export?category=overseas_invoice", headers=auth_headers
    )
    assert response.status_code == 200
    # CSV body should contain the overseas invoice but not the VAT one
    body_text = response.content.decode("utf-8-sig")
    assert "INV-OVERSEAS-E" in body_text
