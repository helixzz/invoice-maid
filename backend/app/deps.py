...
async def get_admin_user(user: Annotated[User, Depends(get_current_user)]) -> User:
    ...
    Raises 403 — not 404 — because the caller is authenticated and the
    endpoint's existence is public (it's in the OpenAPI schema); what
    the server is refusing is the action, not leaking whether the
    endpoint exists. This is the opposite of ``assert_owned``'s 404:
    there, the caller might be asking about a resource they don't own,
    and we hide existence. Here, the endpoint is well-known; we just
    enforce the role."""
